...
<div x-data="setup()" :class="{ 'dark': isDark }">
    <div class="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-white dark:bg-gray-700 text-black dark:text-white">
        ...
        <section class="max-w-4xl p-6 mx-auto bg-gray-400 rounded-md shadow-md my-7">
            <h1 class="text-4xl font-bold capitalize">Edit tag</h1>
            <form method="POST" action="<?=URLROOT?>tagController/UpdateTag/<?= $data['tag']->tag_id; ?>" class="flex flex-col gap-5">
                <div class="grid grid-cols-1 gap-1 mt-4 sm:grid-cols-2">
                    <div>
                        <label class="font-bold" for="name">tag Name</label>
                        <input id="name" name="tagName" value="<?= $data['tag']->tag_name; ?>" type="text" class="block w-full px-4 py-2 mt-2 text-gray-400 bg-white border border-[#685942] rounded-md focus:border-[#685942] focus:outline-none focus:ring">
